...
*   Lista com Nó Descritor em C para a aula de Algoritmos e Estrutura de Dados 1
*/
...
int insere_lista_inicio(nD *main, int x);
int remove_lista_final(nD *main);
nD* cria_lista();
...
int remove_lista_final(nD *main){
    if(main->inicio){
        Lista *ant, *aux = main->inicio;
        while(aux->prox){
            ant = aux;
            aux = aux->prox;
        }
        int retorno = aux->info;
        if(aux == main->inicio){    //  remover o primeiro?
