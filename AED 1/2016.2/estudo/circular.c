...
void push(int valor){//adicionar no inicio da lista
	...
	if(primeiro == NULL)
        ...
        while(primeiro->ant != NULL){	//	enquanto o elemento da direita não for nulo
            primeiro = primeiro->ant; //p passa para o próximo elemento
        }
	printf("primeiro: %i", primeiro);
	ponto *p; //auxiliar
...
void print(){	//	imprime em ordem crescente
	...
	if(lista == NULL)//lista vazia
		...
		do{	//	enquanto não houver objetos nulos, da direita
			printf("\nElemento: %d\n",p->x);
